...
// Generated from: storagegroup/types.proto
...
export namespace NeoFsV2Storagegroup {

export interface StorageGroup {
  ValidationDataSize: bigint;
  ValidationHash?: NeoFsV2Refs.Checksum;
  ExpirationEpoch: bigint;
  Members: NeoFsV2Refs.ObjectID[];
}

export class StorageGroupImpl implements StorageGroup {
  ValidationDataSize!: bigint;
  ValidationHash?: NeoFsV2Refs.Checksum;
  ExpirationEpoch!: bigint;
  Members!: NeoFsV2Refs.ObjectID[];

  constructor(data?: Partial<StorageGroup>) {
    this.ValidationDataSize = data?.ValidationDataSize ?? 0n;
    this.ValidationHash = data?.ValidationHash ?? undefined;
    this.ExpirationEpoch = data?.ExpirationEpoch ?? 0n;
  ...
  serializeBinary(): Uint8Array {
    ...
    if (this.ValidationDataSize !== 0n) {
      writer.writeUint64(1, this.ValidationDataSize);
    }
    if (this.ValidationHash) {
      writer.writeMessage(2, this.ValidationHash);
    }
    if (this.ExpirationEpoch !== 0n) {
      writer.writeUint64(3, this.ExpirationEpoch);
    }
  ...
  static deserializeBinary(data: Uint8Array): StorageGroupImpl {
    const reader = new BinaryReader(data);
    const message = new StorageGroupImpl();
    ...
    while (reader.position < reader.buffer.length) {
      ...
      switch (fieldNumber) {
        case 1: // ValidationDataSize
          if (wireType === 0) { // Varint
            message.ValidationDataSize = reader.readUint64();
          } else if (wireType === 1) { // 64-bit
            message.ValidationDataSize = reader.readUint64Fixed();
          } else {
            reader.skipField(wireType);
          }
          break;
        case 2: // ValidationHash
          if (wireType === 2) { // Length-delimited
            message.ValidationHash = reader.readMessage(NeoFsV2Refs.ChecksumImpl.deserializeBinary);
          } else {
            reader.skipField(wireType);
          }
          break;
        case 3: // ExpirationEpoch
          if (wireType === 0) { // Varint
            message.ExpirationEpoch = reader.readUint64();
          } else if (wireType === 1) { // 64-bit
            message.ExpirationEpoch = reader.readUint64Fixed();
          } else {
            reader.skipField(wireType);
          }
          break;
  ...
  toObject(): StorageGroup {
    return {
      ValidationDataSize: this.ValidationDataSize,
      ValidationHash: this.ValidationHash,
      ExpirationEpoch: this.ExpirationEpoch,
